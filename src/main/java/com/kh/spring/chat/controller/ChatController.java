...
public class ChatController {
	...
	public String openChatRoom(
		...
		int chatRoomNo = service.openChatRoom(room); // 생성된 채팅방 번호 (db에 생성된 번호)
		
		String path = "redirect:/chat/";
		if(chatRoomNo > 0) { // 제대로 생성이 되었음!
			ra.addFlashAttribute("alertMsg" , "채팅방 생성 성공!");
			path += "chatRoomList";
			// path += "room/"+chatRoomNo; // 상세화면 구현 후 변경 예정
		} else {
			ra.addFlashAttribute("alertMsg" , "채팅방 생성 실패"); // addAttribute -> session scope에 잠시 담았다가 request scope에 redirect 후 넘겨줌.
			path += "chatRoomList";
		}
	...
	public String joinChatRoom(
				@ModelAttribute("loginUser") Member loginUser,
				// session scope에 있는 loginUser를 넣어준다.
		...
		if(list != null) {
			...
			ra.addFlashAttribute("alert","채팅방이 존재하지 않습니다!");
